...
public class C0801_08 {
	...
	public static void main(String[] args) {
		// list -> 순서O, 중복O
		// Set ->  순서X, 중복X 2개테이블 합쳤을때 동일한 것은 1개만 입력되도록 할때
		...
		// Iterator 사용 - 1회성
		Iterator it = set.iterator(); // list,set iterator사용해야 함.
		while(it.hasNext()) { // hasNext() 다음것이 있는지 확인
			System.out.println(it.next()); //next() 1개 가져옴.
		}
		Iterator it2 = set.iterator(); // list,set iterator사용해야 함.
		System.out.println("------------------");
		...
		list.add(4);
		list.add(1);
		Iterator it3 = list.iterator(); // list,set iterator사용해야 함.
		System.out.println("------------------");
